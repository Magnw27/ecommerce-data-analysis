...
def fetch_market_data(period: str = "1y", interval: str = "1d") -> pd.DataFrame:
    ...
    response = requests.get(MARKET_API.format(ticker=TICKER), params=params, timeout=20, headers={"User-Agent": "Magnw27-NVDA-Market-Lab/2.0"})
...
def main() -> None:
    ...
    path = save_market_data(fetch_market_data(args.period, args.interval), Path(args.output))
